...
#
# macOS flow:
#   We create a writable user-local copy of QQ.app, overlay the NapCat/QCE
#   runtime into that bundle, patch its package.json `main` to point at
#   loadNapCat.js, then launch the copied QQ binary with `--no-sandbox`.
#   This avoids modifying /Applications/QQ.app directly.
...
: "${QCE_QUICK_LOGIN_UIN:=}"
if [ -n "${QCE_QUICK_LOGIN_UIN}" ] && [ -z "${NAPCAT_QUICK_ACCOUNT:-}" ]; then
    export NAPCAT_QUICK_ACCOUNT="$QCE_QUICK_LOGIN_UIN"
fi

if [[ "${OSTYPE:-}" == darwin* ]]; then
    NAPCAT_QQ_PATH="$("$SCRIPT_DIR/prepare-local-qq-app.sh")"
...
if [[ "$NAPCAT_QQ_PATH" == *.app/Contents/MacOS/* ]]; then
    QQ_PKG_JSON="$(cd "$QQ_DIR/../Resources/app" 2>/dev/null && pwd)/package.json"
else
...
ensure_macos_native_addons_signed() {
    if ! command -v codesign >/dev/null 2>&1; then
        echo "[Warning] codesign not found; skipping native addon signature checks."
        return
    fi

    local addon
    while IFS= read -r -d '' addon; do
        if codesign -v "$addon" >/dev/null 2>&1; then
            continue
        fi
        echo "[Info] Ad-hoc signing native addon: $addon"
        codesign --force --sign - "$addon" >/dev/null 2>&1 || {
            echo "[Error] Failed to sign native addon: $addon"
            exit 1
        }
        xattr -dr com.apple.quarantine "$addon" >/dev/null 2>&1 || true
    done < <(find "$SCRIPT_DIR/native" -type f \( -name '*.node' -o -name '*.dylib' \) -print0 2>/dev/null)
}
...
QQ_ARGS=(--no-sandbox)
if [ -n "${NAPCAT_QUICK_ACCOUNT:-}" ]; then
    echo "[Info] Using quick login UIN: $NAPCAT_QUICK_ACCOUNT"
    QQ_ARGS+=(-q "$NAPCAT_QUICK_ACCOUNT")
fi

exec "$NAPCAT_QQ_PATH" "${QQ_ARGS[@]}"
